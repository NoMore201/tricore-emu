...
void Tricore::Iom::read(std::byte *buffer_out, u32 address, usize length) {
    ...
    case reg_iom_clc_offset: {
            ...
            const auto *range_start = reinterpret_cast<std::byte *>(&m_iom_clc);
...
void Tricore::Iom::write(const std::byte *buffer_in, u32 address,
                         usize length) {
    const u32 offset = address - iom_memory_start_address;
    switch (offset) {
    case reg_iom_clc_offset: {
