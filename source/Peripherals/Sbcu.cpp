...
void Tricore::Sbcu::read(std::byte *buffer_out, u32 address, usize length) {
    ...
    case reg_sbcu_id_offset: {
            ...
            const auto *range_start = reinterpret_cast<std::byte *>(&m_sbcu_id);
...
void Tricore::Sbcu::write(const std::byte *buffer_in, u32 address,
                         usize length) {
    const u32 offset = address - sbcu_memory_start_address;
    switch (offset) {
    case reg_sbcu_id_offset: {
