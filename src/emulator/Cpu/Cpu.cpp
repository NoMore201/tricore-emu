...
Tricore::Cpu::Cpu(Bus &bus) : m_bus(bus.create_request_handler()) {}
...
void Tricore::Cpu::insn_lea_bol() {
  ...
  u32 effective_address =
      m_addr_reg[format.b] + Utils::sign_extend32<16>(format.off16);
  // A[a] = EA[31:0]
...
void Tricore::Cpu::insn_sh_src() {
  ...
  const i32 sign_extended_const4 =
      static_cast<i32>(Utils::sign_extend32<4>(format.const4));
  if (sign_extended_const4 >= 0) {
    ...
    spdlog::trace("==> Cpu: SH shift right by {} value 0x{:08X}",
                  -sign_extended_const4, m_data_reg[format.a]);
    m_data_reg[format.a] =
        m_data_reg[format.a] >> static_cast<u32>(-sign_extended_const4);
  }
...
void Tricore::Cpu::insn_insert_rcpw() {
  ...
  RcpwFormatParser{insn}.parse(
      [this](u32 index_a, u32 const4, u32 width, u32 pos, u32 index_c) {
        ...
        m_data_reg[index_c] =
            Utils::deposit32(const4, pos, width, m_data_reg[index_a]);
      });
...
void Tricore::Cpu::insn_stw_ssr() {
  ...
  m_bus.write32(output, m_addr_reg[format.b]);
...
void Tricore::Cpu::insn_sh_rc() {
  ...
  if (sign_extended_const9 >= 0) {
    ...
    spdlog::trace("==> Cpu: SH shift right value 0x{:08X} of {} positions",
                  m_data_reg[parser.a], -sign_extended_const9);
    m_data_reg[parser.c] =
        m_data_reg[parser.a] >> static_cast<u32>(-sign_extended_const9);
  }
