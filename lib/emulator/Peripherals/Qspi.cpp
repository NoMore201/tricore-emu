...
void Tricore::Qspi::read(std::byte *buffer_out, u32 address, usize length) {
    ...
    case reg_qspi0_clc_offset: {
            ...
            const auto *range_start = reinterpret_cast<std::byte *>(&m_qspi0_clc);
...
void Tricore::Qspi::write(const std::byte *buffer_in, u32 address,
                         usize length) {
    const u32 offset = address - qspi_memory_start_address;
    switch (offset) {
    case reg_qspi0_clc_offset: {
