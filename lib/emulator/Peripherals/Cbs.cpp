...
void Tricore::Cbs::read(std::byte *buffer_out, u32 address, usize length) {
    ...
    case reg_cbs_jdpid_offset: {
            ...
            const auto *range_start = reinterpret_cast<std::byte *>(&m_cbs_jdpid);
...
void Tricore::Cbs::write(const std::byte *buffer_in, u32 address,
                         usize length) {
    const u32 offset = address - cbs_memory_start_address;
    switch (offset) {
    case reg_cbs_jdpid_offset: {
