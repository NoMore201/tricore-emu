...
void Tricore::Can::read(std::byte *buffer_out, u32 address, usize length) {
    ...
    case reg_can0_clc_offset: {
            ...
            const auto *range_start = reinterpret_cast<std::byte *>(&m_can0_clc);
...
void Tricore::Can::write(const std::byte *buffer_in, u32 address,
                         usize length) {
    const u32 offset = address - can_memory_start_address;
    switch (offset) {
    case reg_can0_clc_offset: {
